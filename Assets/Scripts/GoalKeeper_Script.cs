...
public class GoalKeeper_Script : MonoBehaviour {
	...
	void Start () {
		...
		GetComponent<Animation>()["goalkeeper_high_right"].speed = 1.0f; 
		GetComponent<Animation>()["goalkeeper_high_left"].speed = 1.0f; 
		GetComponent<Animation>()["goalkeeper_low_right"].speed = 1.0f; 
		GetComponent<Animation>()["goalkeeper_low_left"].speed = 1.0f; 
		...
		switch (state) {
			...
			case GoalKeeper_State.RESTING:
				...
				if ( !GetComponent<Animation>().IsPlaying("guard") )
					GetComponent<Animation>().Play("guard"); 
			...
			case GoalKeeper_State.STOLE_BALL:
				...
				{
					// Check if the goalkeeper is outside the field boundaries



					Vector3 clampedPosition = fieldBounds.GetComponent<Collider>().ClosestPoint(transform.position);
